...
for x in range(len(columns['Body'])):
        urls = re.findall('https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+', columns['Body'][x])
        for a in urls:
            url.append(a)
...
for x in url:
    if x not in url1:
        if x not in common_urls: 
            if x not in common_urls1:
                url1.append(x)
